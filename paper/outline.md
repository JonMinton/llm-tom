...
**Update 2026-06-29 (revised, identification-aware design — see `prereg/notes/p3-synthesis.md`).** The original single composite "substrate-distance" axis was found **non-identifying** by an adversarial fresh-eyes review (it conflated surface novelty with structural novelty, and neither flat nor decaying NCME was interpretable). The revised probe: (i) decomposes into a **Surface-Novelty × Structural-Novelty** factorial with the same content rendered across surface forms; (ii) replaces the failed-to-reject rule with **equivalence testing** (ROPE/TOST — B1 must positively clear an invariance band; "inconclusive" is a real verdict); (iii) adds an **upstream encoding-gate** (condition NCME on trials whose surface was provably encoded at an early layer) and **Fieller** ratio intervals, *dropping* the survivorship-biasing downstream denominator gate; (iv) adds a **Lipschitz-bound-violation** test that turns flatness into a positive B1 signature. **Framing shift:** the probe is now presented as achieving **partial identification** (conditional on the encoding controls), and the original confound-discovery becomes a *citable contribution* ("earlier formulations treated surface and structural novelty as a single composite axis, which is non-identifying because LLM representational geometry tracks surface frequency rather than human semantic taxonomy"), with a clearly-marked residual-identification-limit section. This subsection is no longer "the most attackable as an unexamined claim" — its attackability has been exercised and converted into rigour + honesty.
...
### Significance and potential impact — assistance under vague intent *(DRAFT, added 2026-06-29; challenge freely)*

**Claim (potential, contingent on the experiment being executed).** The B1/B2 diagnostic predicts *where* a model's capacity to infer a user's latent intent generalises versus silently fails — directly relevant to AI assistance on **underspecified requests**, where the user is implicitly helping to repartition a shared hypothesis space rather than issuing a fully-formed query. This is the constructive counterpart to the defensive deployment-reply in § 9 above.

- **Beneficiary + mechanism (UK Pathways-to-Impact framing).** Beneficiaries: users making vague/exploratory requests; designers of assistant systems. Mechanism: a *generalisation diagnostic*. A model best characterised by **B1** (substrate-independent primitive) should sustain collaborative intent-inference into unfamiliar domains; one characterised by **B2** (interpolator) should do so only near the training manifold, degrading smoothly — and (per the novel-condition quantisation confound surfaced in the P3 dialogue) can *look* competent while failing on the genuinely novel cases. Knowing which regime a deployed model occupies tells designers when to trust implicit inference and when to add explicit clarifying-question scaffolding.
- **The bridge is IA-ε (pragmatic stance).** The 20-Questions-derived stance axis (collaborative / literal / adversarial) operationalises exactly the "help me work out what I'm even asking" case: a genuine divergent-state primitive should be stance-sensitive in the theoretically expected direction (collaborative → more informative repartitioning); an interpolator's stance-sensitivity should decay off-distribution. This reconnects the mechanism to the motivating phenomenon (§ 1.1) and the construct-validity bridge (§ 10, "from 20 Questions to single-turn stimuli").
- **In-scope framing (honesty discipline).** The claim is about the **reliability and diagnosability of assistant behaviour**, not machine "understanding" of the user. Stated maximally honestly: the method predicts *where an assistant's intent-inference holds and where it breaks*, not that the model has a theory of the user's mind. Over-stating this would violate the scope discipline (§ 9 framing) and invite the deflationary counter.
- **Status.** For a design/position paper this is *potential significance* — a pathway, not a result — and must be reported as contingent on the pre-registered experiment being run downstream.
