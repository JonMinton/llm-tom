...
**Host confirmation (2026-06-30).** The first run executed on the laptop; the pilot was then **independently re-run on the butler — the actual M4 Mac mini** (Darwin arm64) — from a fresh `venv` (torch 2.12.1, transformer-lens 3.5.0). Every number below reproduced **exactly** (deterministic, seed 0) across three butler runs, confirming the result on the intended host.

The wall-clock, however, varied wildly (41 → 30 → 10 min) and was investigated — it is **host contention, not config**. Benchmarks rule out the obvious suspects: a pythia-160m forward is ≈ **60–70 ms flat across 1–10 torch threads** (torch already defaults to 4; `run_with_cache` is ~5% over a plain forward), so threading is a non-lever; and a sustained probe held **~58 ms/pass flat for 40 s with no clock-down**, ruling out thermal throttling. Instead the butler — an always-on macOS *desktop* — intermittently **starves** a long compute job of CPU: the same probe then stalled to ~12 s/pass for ~5 min when other work seized the cores. **Killing Steam cut the pilot 30 → 10 min (3×);** the residual stalls trace to background system maintenance (Spotlight `mds`, and a pending OS update — `softwareupdated`/`MobileSoftwareUpdate` active; load avg ~4 with no single hot process). The compute *floor* is ~2.5 min (~70 ms × ~2,200 passes); the deterministic numbers are host-independent. For heavier Stage-2 runs, treat the butler as a **shared** box (quiet it / let updates finish, or budget for variable wall-clock); the code-side lever for scale is **batching**, not threads.
