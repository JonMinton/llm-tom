...
**Reproduced on the butler M4 (2026-06-30).** The pilot was independently re-run on the butler — the actual M4 Mac mini — from a fresh venv; every number above reproduced **exactly** (deterministic), across three runs. The original "M4 pilot" run had executed on the laptop; this confirms the result on the intended host.

**Wall-clock investigated and corrected (2026-06-30).** An earlier draft of this note blamed the butler's slow wall-clock (41 → 30 → 10 min) on single-thread torch and prescribed `OMP_NUM_THREADS` — **that was wrong, and is retracted.** Benchmarks show a pythia-160m forward is ~60–70 ms **flat across 1–10 torch threads** (threading is a non-lever; torch already defaults to 4) and held **flat for 40 s sustained** (not thermal throttling). The real cause is **host contention**: the butler is an always-on macOS desktop whose background load intermittently *starves* long compute jobs of CPU (a probe ran clean at 58 ms/pass then stalled to ~12 s/pass for ~5 min). **Killing Steam (Jon-authorised) cut the pilot 30 → 10 min (3×);** the residual ~4× over the ~2.5 min compute floor traces to system maintenance (Spotlight `mds` + a pending OS update). The deterministic numbers are host-independent. Code-side lever for scale = **batching**, not threads. Corrected detail in `implementation/pilot/m4-pilot-results.md`.
